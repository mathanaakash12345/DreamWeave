...
# Declare a global list variable
global prompt
prompt=""
...
class HandTracker:
    ...
    def getUpFingers(self, img):
        ...
        upfingers = []
        if pos:
            upfingers.append((pos[4][1] < pos[3][1] and abs(pos[4][0] - pos[3][0]) > 20))  # Thumb
            upfingers.append((pos[8][1] < pos[7][1] and pos[7][1] < pos[6][1]))  # Index finger
            upfingers.append((pos[12][1] < pos[11][1] and pos[11][1] < pos[10][1]))  # Middle finger
            upfingers.append((pos[16][1] < pos[15][1] and pos[15][1] < pos[14][1]))  # Ring finger
            upfingers.append((pos[20][1] < pos[19][1] and pos[19][1] < pos[18][1]))  # Pinky
        return upfingers
...
# Initialize the hand tracker
    ...
    for _ in range(5):
        ...
        colors.append(ColorRect(300 + len(colors) * 100, 0, 100, 100, (b, g, r)))
    colors.append(ColorRect(800, 0, 100, 100, (0, 0, 0), "Eraser"))

    clear = ColorRect(900, 0, 100, 100, (100, 100, 100), "Clear")

    # Save button
    saveBtn = ColorRect(1000, 0, 100, 100, (0, 255, 0), "Save")
        ...
        if upFingers:
            ...
            if upFingers[1] and not whiteBoard.isOver(x, y):
                ...
                if saveBtn.isOver(x, y) and not coolingCounter:
                    coolingCounter = 10
                    # Save the canvas
                    white_canvas = np.ones((720, 1280, 3), np.uint8) * 255
                    inv_canvas = cv2.bitwise_not(canvas)
                    save_img = cv2.bitwise_or(white_canvas, inv_canvas)
                    file_name = os.path.join(save_dir, "drawing.png")
                    cv2.imwrite(file_name, inv_canvas)
                    print(f"Canvas saved as {file_name}")

                    # Generate and print caption
                    # caption = generate_caption(file_name)
                    caption=",best quality and a detailed description for the given scribble."
                    print(f"Prompt for the Scribble: {caption}")
                    print(prompt)
